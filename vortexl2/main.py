...
def handle_logs(manager: ConfigManager):
    ...
    # Add forward services for all tunnels (new naming + legacy fallback)
    for config in manager.get_all_tunnels():
        fwd = ForwardManager(config)
        for port in config.forwarded_ports:
            services.append(fwd._get_service_name(port))
            services.append(fwd._legacy_service_name(port))
